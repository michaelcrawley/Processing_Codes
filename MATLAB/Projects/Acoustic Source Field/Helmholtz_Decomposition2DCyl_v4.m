function [potential,solenoidal] = Helmholtz_Decomposition2DCyl_v4(z,r,Uz,Ur)
...
%     A(1:M:1+(N-1)*M,1:M:1+(N-1)*M) = spdiags(repmat(zcoefs,N,1),(-L:L)*M, N,N) + spdiags(repmat([2 -2 0]/dr/dr,N,1),(-L:L), N,N);
    ...
    %Inflow - (:,1)
    A(1,1:M+1) = sparse([-2/dr/dr-2/dz/dz,2/dr/dr,zeros(1,M-2),2/dz/dz]);
    for n = 2:M-1
        A(n,n-1:M+n) = [1/dr/dr-1/2/dr/r(n),-2/dz/dz-2/dr/dr,1/dr/dr+1/2/dr/r(n),zeros(1,M-2),1/dz/dz];
    end

    %Outflow - (:,N)
    br_corner = 1+(N-1)*M;
    tr_corner = N*M;
    A(br_corner,br_corner-M:br_corner+1) = [2/dz/dz,zeros(1,M-1),-2/dz/dz-2/dr/dr,2/dr/dr];
    for n = br_corner+1:tr_corner-1
        A(n,n-M:n+1) = [2/dz/dz,zeros(1,M-2),1/dr/dr-1/2/dr/r(n),-2/dz/dz-2/dr/dr,1/dr/dr+1/2/dr/r(n)];
    end
    A(tr_corner,tr_corner-M:tr_corner) = [2/dz/dz,zeros(1,M-2),2/dr/dr,-2/dr/dr-2/dz/dz];    
    %
    phi = reshape(A\F,M,N);
