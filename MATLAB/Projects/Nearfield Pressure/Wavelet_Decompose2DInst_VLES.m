function Wavelet_Decompose2DInst_VLES(src_dir,flist,out_dir,optset,cpus)
    tic;
    ...
    if ~isfield(optset,'param'), optset.param = [6 6]; end
...
%     fields = [1 2 3 7 14];
    ...
    for k = 1:length(flist)
        ...
        %remove mean so signal is only fluctuating component - note that
        %this assumes only one block is present!!!
        sig = sig-repmat(mean(sig,1),[size(sig,1),1]);
        
        %cut cutoff phase velocity
        ...
        intwaveform = zeros(N(1),length(int_x));
        ...
        if k == 1
            ...
            wndo_t = repmat(optset.windowfun(N(1)),[1 length(int_x)]); %window along t-dimension (DIM1)
        end
        ...
        if optset.Nx > -1
            ...
            for q = 1:N(1)
                tmp(q,:) = interp1(x,sig(q,:),int_x,'pchip');
            end
            intwaveform = tmp;
        else
            intwaveform = sig;
        ...
        tmp = intwaveform.*wndo_t;
        swap = zeros(L(1),L(2));
        swap(1:N(1),1:length(int_x)) = tmp;
        tmp = swap;        

        %Filtered Reconstructions
        xh = fftn(tmp);
        tmp = ifftn(xh.*conj(sub_filt))/C_delta;
        subsonic = real(tmp(1:N(1),1:length(int_x)));
        tmp = ifftn(xh.*conj(sup_filt))/C_delta;
        supersonic = real(tmp(1:N(1),1:length(int_x)));
    ...
    switch dim
        ...
        case 'ST2'
            switch lower(mother)
                ...
                case 'paul'
                    if isempty(m), m = 4; end
                    fcoef = NaN; %need to find this number!!!
                    wavelet = @(omega,k) (2^m)/sqrt(m*factorial(2*m-1))*(((omegak.^2+k.^2)/2).^m).*exp(-0.5*(omegak.^2+k.^2)).*(omega > 0);
                otherwise
                    error('Undefined Mother Wavelet');
            end
        otherwise
