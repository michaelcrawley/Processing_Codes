function Wavelet_Decompose2DInst_V2(src_dir,flist,out_dir,a,windowfun,flag,cpus)
    ...
    for k = 1:length(flist)
        ...
        if k == 1
            ...
            %are spatial
            [sub_filt,sup_filt,C_delta,total_filt] = DaughterWavelets(L,[dt dx],dj,a,mother,param);
        end
...
function [sub_filt,sup_filt,C_delta,total] = DaughterWavelets(L,dt,dj,ca,mother,param)
    ...
    sub_filt = zeros(L(1),L(2),length(scales{1}(1:sub_indx)),length(scales{2}));
    tic;
    disp('Computing Subsonic Map....');
    for j = 1:sub_indx
        for k = 1:sN1
%             wavelet = MotherWavelets('ST2',mother,param);
            sub_filt(:,:,j,k) = wavelet(scales{2}(k)*T/sqrt(scales{1}(j)),scales{2}(k)*K*sqrt(scales{1}(j)))/(scales{1}(j)*scales{2}(k));
        end
    end
    sub_filt = trapz(scales{2},sub_filt,4);
    sub_filt = trapz(scales{1}(1:sub_indx),sub_filt,3);
    disp(['Complete...',' Processing time: ',num2str(round(toc)),' seconds...']);
    
    %Calculate sonic/supersonic-filtered map
    sup_filt = zeros(L(1),L(2),length(scales{1}(1:(cN1-sub_indx))),length(scales{2}));
    tic;
    disp('Computing Supersonic Map...');
    for j = 1:cN1-sub_indx
        for k = 1:sN1
%             wavelet = MotherWavelets('ST2',mother,param);
            sup_filt(:,:,j,k) = wavelet(scales{2}(k)*T/sqrt(scales{1}(j+sub_indx)),scales{2}(k)*K*sqrt(scales{1}(j+sub_indx)))/(scales{1}(j+sub_indx)*scales{2}(k));
        end
    end
    sup_filt = trapz(scales{2},sup_filt,4);
    sup_filt = trapz(scales{1}(sub_indx+1:end),sup_filt,3);
