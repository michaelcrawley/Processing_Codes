function SoundProcess_v12b(PARAMS,savedir)
%This function has been modified by M Crawley to alter the manner in which
%the data is saved.  The dialog box asking if the user inputs are correct
%(Lines 299-305) has been removed.  An additional optional input (savedir)
%has been included, which allows the user to change the save directory of
%the data.  New subfolders for saving data are created (see line 427).  An
%additional .mat file consisting of all data in the fft, S.fft, and .set
%files is created and saved (see line 931).  All modifications are noted
%with the phrase [Modification by M Crawley].
...
%       mV/Pa and calibration data at 316 mV/Pa, set 'dBShift' to 10.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
for n = 1:Nch   %Calculates spectral reference value for each channel from the calibration files
	...
	if isbinary
		fid = fopen([CalPath filesep CalFiles{n}],'r');
		tmp = fread(fid,'float32'); fclose(fid);	%Assumes only one channel of data in calibration files
	else
...
%%% Create save folders [Modification by M Crawley]
tstamp = now;
if ~exist('savedir','var')
    out_dir = [src_dir filesep datestr(tstamp,30)];
else
    out_dir = [savedir filesep datestr(tstamp,30)];
end
mkdir(out_dir);
figdir = [out_dir filesep 'Figures'];
fftdir = [out_dir filesep 'FFT'];
matdir = [out_dir filesep 'Mat Files'];
mkdir(figdir); %create folder for png and fig files
mkdir(fftdir); %create folder for fft and S.fft bin files
mkdir(matdir); %create folder for .mat files
...
%             dBCorrected_DT(:,m) = ToneSubtract_v5(fx,dBCorrected(:,m),'Moving',0,10);
    ...
    for m = 1:Nch   %Plots smoothed spectra on top of original spectra
        semilogx(Std,dBCorrected_DT(:,m),color(mod(m-1,7)+1));
    ...
    saveas(gcf,[figdir filesep Fnm(1:end-4),'.fig']);  %saveas(gcf,[out_dir filesep Fnm(1:end-4) '.fig']);   %Saves figure as .fig [Modification by M Crawley]
    saveFigure_v2(gcf,[figdir filesep Fnm(1:end-4)],300)  %saveFigure_v2(gcf,[out_dir filesep Fnm(1:end-4)],300); %Saves figure as .png [Modification by M Crawley]
    close    %Closes figure
    
    %%%%%  SPECTRAL DATA FILES  %%%%%%%%%%%%%%%%%%%%%%%%%%%
    fid = fopen([fftdir filesep Fnm(1:end-3) 'fft' EXT],'w');%fid = fopen([out_dir filesep Fnm(1:end-3) 'fft' EXT],'w');   %Opens .fft* file to write column headers [Modification by M Crawley]
    fprintf(fid,['Frequency (Hz)\tStrouhal Number' delimO LBL '\n']);
    fclose(fid);
    dlmwrite([fftdir filesep Fnm(1:end-3) 'fft' EXT],[fx Std dBCorrected], 'delimiter', delimO, '-append'); %dlmwrite([out_dir filesep Fnm(1:end-3) 'fft' EXT],[fx Std dBCorrected], 'delimiter', delimO, '-append');   %writes spectral data to file [Modification by M Crawley]
    
    fid = fopen([fftdir filesep Fnm(1:end-3) 'S.fft' EXT],'w'); %fid = fopen([out_dir filesep Fnm(1:end-3) 'S.fft' EXT],'w');   %Opens .S.fft* file to write column headers [Modification by M Crawley]
    fprintf(fid,['Frequency (Hz)\tStrouhal Number' delimO LBL '\n']);
    fclose(fid); clear LBL;
    dlmwrite([fftdir filesep Fnm(1:end-3) 'S.fft' EXT],[fx Std dBCorrected_DT], 'delimiter', delimO, '-append'); %dlmwrite([out_dir filesep Fnm(1:end-3) 'S.fft' EXT],[fx Std dBCorrected_DT], 'delimiter', delimO, '-append');   %writes smoothed spectral data to file [Modification by M Crawley]
    ...
    OASPL = 10*log10(trapz(fx(Bg:Ed),10.^(dBCorrected(Bg:Ed,:)/10),1)); %clear dBCorrected; [Modification by M Crawley]
    ...
    if SHarmonics
        ...
        fprintf(fid,['Forcing Strouhal Number:' delimO num2str(StDF) '\n']);
            %Calculates and prints OASPL for all channels
        if DT~=0
            OASPL_DT = 10*log10(trapz(fx(Bg:Ed),10.^(dBCorrected_DT(Bg:Ed,:)/10),1)); %clear dBCorrected_DT; [Modification by M Crawley]
        ...
        if sum(kp) > 0  %If baseline SET file(s) are found
            ...
            %Calculates and prints OASPL for all channels    
        OASPL_DT = 10*log10(trapz(fx(Bg:Ed),10.^(dBCorrected_DT(Bg:Ed,:)/10),1)); %clear dBCorrected; [Modification by M Crawley]
    ...
    %%% Additional save outputs [Modification by M Crawley]
    processing_params = load(PARAMS);
    TTR = To/(AT+273.15);
    Ue = M*sqrt(1.4*287.05*Temp);
    Mj = M*sqrt(Temp/(AT+273.15));
    rhoe = 1.01e5/(287.05*Temp);
    mue = (1.827e-5*(0.555*291.15+66.667)/(0.555*Temp+66.667).*(Temp/291.15).^(3/2));
    data = struct('Std_axis',Std,'F_axis',fx,'Spectra',dBCorrected,'Spectra_DT',dBCorrected_DT,'To',To,'Te',Temp,'TTR',TTR,'Ue',Ue,'Mj',Mj,'density',rhoe,'viscosity',mue,'Re',ReN);
    if SHarmonics
       data.Stdf = StDF;
       data.ff = FH;
    end    
    results = struct('AAE',AAE,'AAE_DT',AAE_DT,'NJP',NJP,'NJP_DT',NJP_DT,'OASPL',OASPL,'OASPL_DT',OASPL_DT);
    if exist('OASPLB','var')
        results.dOASPL = OASPL - OASPLB;
        results.dAAE = AAE-AAEB;
        results.dNJP = NJP-NJPB;
        if DT ~=0
            results.dOASPL_DT = OASPL_DT - OASPLB_DT;
            results.dAAE_DT = AAE_DT-AAEB_DT;
            results.dNJP_DT = NJP_DT-NJPB_DT;
        end            
    end
    
    tempstr = strrep(strrep(strrep(strrep(Fnm(1:end-4),'.','_'),'-','n'),'(','_'),')','_');
    tempvar = genvarname(tempstr);
    tempdata = struct('processing_params',processing_params,'data',data,'results',results);
    eval([tempvar '=tempdata;']);
    save([matdir filesep tempstr '.mat'],tempstr);
    clear dBCorrected dBCorrected_DT;
end
