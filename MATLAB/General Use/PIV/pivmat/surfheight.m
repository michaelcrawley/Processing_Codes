...
for i=1:numel(dr)
    ...
    if any(strncmpi(varargin,'remap',4))
        ...
        if false
            ...
            % determine the rectangle of correct points
            imin = find(~isnan(dhdx(:,end/2)),1,'first')+1;
            imax = find(~isnan(dhdx(:,end/2)),1,'last')-1;
            jmin = find(~isnan(dhdx(end/2,:)),1,'first')+1;
            jmax = find(~isnan(dhdx(end/2,:)),1,'last')-1;
            
            % truncate the field
            dhdx = dhdx(imin:imax, jmin:jmax);
            dhdy = dhdy(imin:imax, jmin:jmax);
            hresult.x = hresult.x(imin:imax);
            hresult.y = hresult.y(jmin:jmax);
            hresult.xtrunc = imin:imax;
            hresult.ytrunc = jmin:jmax;
        ...
        hresult(i).remap = 'Not remapped'; % now default (v2.10)
    end
    ...
    % tests FM juillet 2011: remap back!
%     if any(strncmpi(varargin,'remap',4))
%         h = griddata(xx, yy, h, xxmes, yymes,'cubic');
%     end
