function c = velocityOfAcousticWavesInAir(f,T,P,RH,f0,c0)
...
%	c0 = speed of reference frequency (m/s)
% List of output variables
%   c = the propagation velocities for frequencies(f) (m/s)
...
%Intermediate calculations for nitrogen and oxygen
%from ISO 9613-1, 6.2, part of eq.5
x_O = 0.01275*exp(-2239.1/T_kel)*fr_O^2./(fr_O^2+f.^2)/(fr_O^2+f0^2);
x_N = 0.1068*exp(-3352/T_kel)*fr_N^2./(fr_N^2+f.^2)/(fr_N^2+f0^2);

%Atmospheric attenuation coefficient, alpha (dB/m)
%from ISO 9613-1, 6.2, eq.5
alpha = (f0^2-f.^2).*(T_rel^(-5/2).*(x_O +x_N))/(2*pi);

c = c0./(1+alpha*c0);	%propagation velocities (m/s)
...
function alpha = standardDay(f)
%Computes absorption for standard day conditions of 15 Celcius, 101.3 kPa,
...
T_kel = 15 +273.15;     %Measured temperature conversion(K)
T_rel = T_kel/T_ref;  %Relative temperature
P_rel = 101.3/P_ref;      %Relative pressure
RH = 0;
