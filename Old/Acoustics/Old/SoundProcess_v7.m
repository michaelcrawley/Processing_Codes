function SoundProcess_v7(M,AT,AP,RH)
...
WNDO = 'hamming';    %Data windowing method
SD = 'F:\Research\Mach0.9\ForcingNoiseData\Array Measurements\Recollecting 200909';   %Starting directory
...
[fx,SPL] = calcSPL(rRaw,sampleRate,dSize,WNDO);
...
for n = 1:length(keep)
    ...
    if DT~=0
        for m = 1:Nch   %Smoothes and removes forcing harmonics from spectrum. If FH==0 (forcing frequency) program doesn't look for harmonics
            dBCal2(:,m) = ToneSubtract_v4(fx,dBCal(:,m),FH,10);
        end
    else
        for m = 1:Nch   %Smoothes forcing harmonics from spectrum.
            dBCal2(:,m) = ToneSubtract_v4(fx,dBCal(:,m),0,10);
        end
    end
    ...
    ylabel('SPL (dB)')
    title({['Average Spectrum for: ' flist{keep(n)}],['Data Normalized for ' num2str(NormD) ' x/D'],['Reynolds Number: ' num2str(ReN,'%.0f')]})
    grid on
    xlim([0.01 4])
    saveFigure_v2(gcf,[dir_name filesep flist{keep(n)}(1:end-4)],300)   %Saves figure as .png
    saveas(gcf,[dir_name filesep flist{keep(n)}(1:end-4) '.fig'])   %Saves figure as .fig
    close    %Closes figure
    ...
    fid = fopen([dir_name filesep flist{keep(n)}(1:end-3) 'S.fftNOS'],'w');   %Opens .HR.fftNOS file to write column headers
    fprintf(fid,['Frequency (Hz)\tStrouhal Number\t' LBL '\n']);
    fclose(fid);
    dlmwrite([dir_name filesep flist{keep(n)}(1:end-3) 'S.fftNOS'],[fx Std dBCal2], 'delimiter', '\t', '-append');   %writes smoothed spectral data to file
