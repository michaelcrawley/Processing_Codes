function SoundProcess_v6(M,AT,AP,RH)
...
for n = 1:Nch   %Calculates spectral reference value for each channel from the calibration files
    rRaw(:,n) = dlmread([PathName FileName{n}],'\t');
end
...
for n = 1:length(keep)
    ...
    grid on
    xlim([0.01 4])
    saveFigure(gcf,[dir_name filesep flist{keep(n)}(1:end-4)],300)   %Saves figure as .png
    ...
    OASPL = 10*log10(trapz(fx(Bg:Ed),10.^(dBCal2(Bg:Ed,:)/10),1));
    for nn = 1:length(ChD)
    ...
    if SHarmonics
        ...
        if sum(kp) > 0  %If baseline SET file(s) are found
            ...
            for nn = 1:length(slist)    %reads baseline SET file(s) and extracts OASPL and total measured energy
                [NOASPLB,VB,kp] = readSETfile([dir_name filesep slist{nn}],[28 29]);
                OASPLB = [OASPLB; VB{1}];
                AAEB = [AAEB; VB{2}];
            end
