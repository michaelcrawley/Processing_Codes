function [Tn Ta] = P1R2(cp,rho, h, k, L, N, To, Tamb,tfinal)
...
%% Numeric Solution 
    ...
    for l = 2:length(t) %set timestepping loop
        ...
        d2Tdx2(N,1:N-1) = 2*Tn(N-1,1:N-1,l-1)/(ds^2)-2*Tn(N,1:N-1,l-1)/(ds^2)+2*(-h/k*(Tn(N,1:N-1,l-1)-Tamb)/ds); %set right convection boundary condition
        d2Tdy2(1:N-1,N) = 2*Tn(1:N-1,N-1,l-1)/(ds^2)-2*Tn(1:N-1,N,l-1)/(ds^2)+2*(-h/k*(Tn(1:N-1,N,l-1)-Tamb)/ds); %set top convection boundary condition
        d2Tdx2(1,1:N-1) = 2*(Tn(2,1:N-1,l-1)-Tn(1,1:N-1,l-1))/(ds^2); %set left boundary condition (dT/dx = 0)
        d2Tdy2(1:N-1,1) = 2*(Tn(1:N-1,2,l-1)-Tn(1:N-1,1,l-1))/(ds^2); %set bottom boundary condition (dT/dy = 0);
        d2Tdx2(N,N) = 2*(Tn(N,N-1,l-1)-Tn(N,N,l-1))/(ds^2)+2*(-h/k*(Tn(N,N,l-1)-Tamb)/ds); %set corner boundary condition
        d2Tdy2(N,N) = 2*(Tn(N-1,N,l-1)-Tn(N,N,l-1))/(ds^2)+2*(-h/k*(Tn(N,N,l-1)-Tamb)/ds); %set corner boundary condition
