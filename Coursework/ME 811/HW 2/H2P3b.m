function [phin Residual] = H2P3b(N,M)
    x = 1/(2*N):1/N:1-1/(2*N);
    dx = mean(diff(x));
    y = 1/(2*M):1/M:1-1/(2*M);
    dy = mean(diff(y));
    ...
    counter = 0;
    I = spdiags(ones(N*M,1),0,N*M,N*M);
    while Residual(counter+1)>Rtol && counter <= itrmax
        phin = ADI2d(I-dt/(rhoc*dx*dy)*X,reshape(phin-dt/(rhoc*dx*dy)*S,M,N),'-TDMA',Residual(counter+1)/10);
        phin = reshape(phin,M*N,1);
        counter = counter +1;
