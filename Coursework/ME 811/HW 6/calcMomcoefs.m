function [Ax Ay] = calcMomcoefs(uf,vf,rho,P,gamma,Uin,dx,dy,xstepi,ystepi)
    ...
    ce = rho.*uf.e; %need to modify rho for weakly compressible flows
    cw = rho.*uf.w;
    cn = rho.*vf.n;
    cs = rho.*vf.s;
    ...
    Ax.P(ystepi+1:end,1) = Ax.P(ystepi+1:end,1)+rho(ystepi+1:end,1)*(Uin^2)*dy+8*gamma*Uin/3/dx*dy;
    ...
    %Outlet boundary
    Ax.O(:,end) = Ax.O(:,end) -dep(:,end)*dy-gamma*dy/dx+rho(:,end).*uf.e(:,end)*dy;
    Ax.E(:,end) = 0;
