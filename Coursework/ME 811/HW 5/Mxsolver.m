function [uh Resx] = Mxsolver(alpha,AO,AE,AW,AN,AS,S,u)
    Rtol = 1E-20;
    itrmax = 2;
    [M N] = size(S);
    ...
    Rx = reshape(S,M*N,1)-Ax*reshape(u(:,2:end),M*N,1);
    Sx = reshape(Rx,M,N);
    Resx = norm(Rx);
    
    %Solve for u hat
%     [uh R] = ADIp(Ax,Sx,'-TDMA',Rtol,itrmax);
    duh = ADIc( AN , AW , (1+alpha)*AO , AE , AS , Rx , zeros(M,N) , -inf , itrmax );

    %append boundary condition
    uh = [zeros(M,1) duh+u(:,2:end)];
end