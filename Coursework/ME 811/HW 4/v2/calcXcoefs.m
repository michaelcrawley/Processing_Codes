...
%     AxE(:,N-1) = 0;    
    ...
    %Modify link coefficients for West Boundary - unnecessary
%     AxW(:,1) = 0;   
    
    %Reshape link coefficients
%     AO = reshape(AO,(N-1)*M,1);
%     AE = reshape(AE,(N-1)*M,1);
%     AW = reshape(AW,(N-1)*M,1);
%     AN = reshape(AN,(N-1)*M,1);
%     AS = reshape(AS,(N-1)*M,1);
%     
%     AN = circshift(AN,1);
%     AS = circshift(AS,-1);%need to check to make sure AN,AS are being shifted properly
%     AE = circshift(AE,M);
%     AW = circshift(AW,-M);
%     
%     Ax = spdiags([AW AS AO AN AE],[-M -1 0 1 M],(N-1)*M,(N-1)*M);
end