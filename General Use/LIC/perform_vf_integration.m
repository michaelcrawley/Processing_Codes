...
if isfield(options, 'pos')
    pos = single(options.pos);
    posx = [];
else
    posx = single(getoptions(options, 'posx', 1:n(1)));
    posy = single(getoptions(options, 'posy', 1:n(2)));
    [Y,X] = meshgrid(posy,posx); posx = length(posx); posy = length(posy); 
    pos = cat(1,X(:)',Y(:)'); clear X Y;
end

M = zeros(size(pos,2),2,m,'single');
for i=1:m
    if verb~=0 && m>1
        progress(i,1,m,verb);
    end
    ...
    if nt>0
        ...
        for k=1:nt
            dx = interp2(1:n(2),1:n(1),vf(:,:,1),pos(2,:),pos(1,:));
            dy = interp2(1:n(2),1:n(1),vf(:,:,2),pos(2,:),pos(1,:));
            pos = pos + DT*cat(1,dx,dy);  % X = X + DT*dx; Y = Y + DT*dy;
            switch bound
                case 'sym'
                    pos(1,:) = clamp(pos(1,:),1,n(1));
                    pos(2,:) = clamp(pos(2,:),1,n(2));
                case 'per'
                    pos(1,:) = mod(pos(1,:)-1,n(1)-1)+1;
                    pos(2,:) = mod(pos(2,:)-1,n(2)-1)+1;
                otherwise
...
M = M(:,:,1:i);
if ~isempty(posx) && (posx~=n(1) || posy~=n(2))
    M = reshape(M, [posx posy 2 m]);
else
    M = reshape(M, [n(1) n(2) 2 m]);
end
