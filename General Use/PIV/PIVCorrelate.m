function [ds,corr,c,vn,vp] = PIVCorrelate(X,Y,W,phD)
% For correlating a segment of a PIV image (moving window
% correlation). Used in conjunction with PIVCorrelationVelocity.m
c = 0;

w = [size(X,1), round(W*1.2)];    %pixels - window dimensions (height, width)
s = 0.1;     %i/pixels - window roll-off rate
S = size(X);    %pixels - image size
[y,x] = meshgrid((1:S(2)),(1:S(1)));

sp = round(w(2)/2);
cs = ceil(w(2)/20);    %window step size
