...
%   matching FILENAME into Mat-files. When used with DaVis files, the
%   resulting Mat files may be re-used under systems without the ReadIMX
%   package.
%
...
%   LOADVEC are accepted (VEC/VC7/IMX/IM7/IMG/TXT/DAT/SET).  Wildcards (*)
...
if nargin==0
    filename={'*.vec','*.vc7','*.imx','*.im7','*.img'};
end

% process numeric input (vec2mat v1.10, from loadvec v1.60):
if isnumeric(filename),
    listfile=[dir('*.VEC') dir('*.VC7') dir('*.TXT') dir('*.DAT') dir('*.IMX') dir('*.IMG') dir('*.IM7') dir('*.CM0') dir('*.UWO')];
