...
lhs1 = (drng_x-0.5)*A.Grid*A.ScaleX(1)+A.ScaleX(2); % x-range
lhs2 = (drng_y-0.5)*A.Grid*A.ScaleY(1)+A.ScaleY(2); % y-range
lhs3 = 0;
lhs4 = 0;
lhs5 = 0;
lhs6 = 0;
...
elseif A.IType==2, % simple 2D vector format: (vx,vy)
	...
	lhs3 = A.Data(:,drng_y   )*A.ScaleI(1)+A.ScaleI(2);
	lhs4 = A.Data(:,drng_y+ny)*A.ScaleI(1)+A.ScaleI(2);
    if A.ScaleY(1)<0.0,
        lhs4 = -lhs4;
    end
    if length(strfind(upper(yes_display_vector),'YES'))>1   % jhkim
...
elseif (A.IType==3 | A.IType==1) , % normal 2D vector format + peak: sel+4*(vx,vy) (+peak)
	...
	for i = 0:5,
		...
		if (i<4) % get best vectors
			dat = A.Data(:,drng_y+(2*i+1)*ny);
			lhs3(mask) = dat(mask);
			dat = A.Data(:,drng_y+(2*i+2)*ny);
			lhs4(mask) = dat(mask);
		else    % get interpolated vectors
			dat = A.Data(:,drng_y+7*ny);
			lhs3(mask) = dat(mask);
			dat = A.Data(:,drng_y+8*ny);
			lhs4(mask) = dat(mask);
		end
    end
...
elseif A.IType==4,
    % Calculate vector position and components
    lhs3 = (drng_z-0.5)*A.Grid*A.ScaleY(1)+A.ScaleY(2);
	lhs4 = A.Data(:,drng_y     )*A.ScaleI(1)+A.ScaleI(2);
	lhs5 = A.Data(:,drng_y+  ny)*A.ScaleI(1)+A.ScaleI(2);
	lhs6 = A.Data(:,drng_y+2*ny)*A.ScaleI(1)+A.ScaleI(2);
...
elseif A.IType==5,
	% Calculate vector position and components
    lhs3 = (drng_z-0.5)*A.Grid*A.ScaleZ(1)+A.ScaleZ(2);
    lhs4 = zeros([nx ny nz]);
    lhs5 = lhs4;
    lhs6 = lhs4;
    for iz=1:nz,
        ...
        for i = 0:5,
            if (i<4) % get best vectors
                mask = (lhs7==(i+1));
                dat = A.Data(:,prange+(3*i+1)*ny);
                px(mask) = dat(mask);
                dat = A.Data(:,prange+(3*i+2)*ny);
                py(mask) = dat(mask);
                dat = A.Data(:,prange+(3*i+3)*ny);
                pz(mask)=dat(mask);
            else    % get interpolated vectors
                mask = (lhs7==(i+1));
                dat = A.Data(:,prange+10*ny);
                px(mask) = dat(mask);
                dat = A.Data(:,prange+11*ny);
                py(mask) = dat(mask);
                dat = A.Data(:,prange+12*ny);
                pz(mask) = dat(mask);
            end
        end
        lhs4(:,:,iz)=px;
