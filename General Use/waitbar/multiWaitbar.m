...
if force || (filled<lastfilled)
    ...
    highlight = [255 - 0.9*(255-entry.CData), 0.9*entry.CData];
    for ii=1:numel( markers )
        progresscdata(:,markers(ii)+[-1,0],:) = highlight;
    end
    
    % Add highlight and shadow to the ends of the bar
    progresscdata(:,1,:) = 255 - 0.4*(255-entry.CData);
    progresscdata(:,filled,:) = 0.4*entry.CData;
    if filled>=4
        progresscdata(:,2,:) = 255 - 0.7*(255-entry.CData);
        progresscdata(:,filled-1,:) = 0.7*entry.CData;
    end
