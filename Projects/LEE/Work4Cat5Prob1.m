function [grab,X,Y] = Work4Cat5Prob1(dx)
...
%Inputs:    dx: normalized grid spacing (axial and radial spacings will be equal)
    ...
    M = uj/a;
    omega_o = 76; %this is normalized
    A = 0.001 * Rhalf/rhoj/(a^3); %this is normalized
    Bx = 0.04*log(2)*Rhalf^2; %normalized
    By = 0.32*log(2)*Rhalf^2; %normalized
    
    %Set up grid
    x = -50:dx:150;
    y = 0:dx:50;
    [X,Y] = meshgrid(x,y);
    forcing = @(t) A*exp(-(Bx*X.^2 + By*Y.^2))*cos(omega_o*t); %normalized forcing function
    
    %Find time-step
    bj = [2.3025580883830 -2.4910075998482 1.5743409331815 -0.38589142217162];
    Omega = timestepping(bj,M,length(x),0.1); %Fuck Tam
    dt = Omega*dx/(1.75*(M + sqrt(2))); %note that this is normalized, hence why the speed of sound is missing, and the normalized dx is used
    ...
    coefs = DRPlookup(1,5,dx)'; %asymmetric coefs for symmetric boundary condition
    dely_ubar = partial_y*u_bar;
    dely_rhobar = partial_y*rho_bar;

    %Boundary condition coefficients
    r = sqrt(X.^2+Y.^2);
    V = u_bar.*(X./r)+sqrt(1-(u_bar.*(Y./r)).^2);
    A = V.*(X./r);
    B = V.*(Y./r);
    C = V./(2*r); 
    ...
    Dv = Drho;
    Dp = Drho;
    grab{1} = zeros(length(y),length(x),imax/downsample); %grab pressure perturbations 
    counter = 1;
