function NF_Preprocessing_EXP(src_dir,flist,cal_dir,out_dir,cpus)
    ...
    %afterwards). Additionally, this will 'resample' the waveforms to a
    %higher frequency (2 MHz) in order to improve the accuracy of the
    %averaged waveforms.
    %Code version: 2.0
    %Output mat-file version: 2.0
...
%     matlabpool(cpus);
    ...
    for n = 1:length(flist)
        ...
        if tl{n}.phys.Stdf ~= 0  %forced cases
            ...
            smp = zeros(pp.sBS*pp.Nresample,pp.sNBmax,pp.NCh-1);
            for nn = 1:pp.NB
                nsblocks = length(a_i{nn});
                tmp = sm_pressure(:,slocator:(nsblocks+slocator-1),:);
                [X,Y,Z] = size(tmp);
                gb = rem(X*Y,pp.sBS*pp.Nresample);
                tmp = reshape(tmp,[X*Y Z]);
                tmp = tmp(1:end-gb,:);
                nblocks = floor(length(tmp)/(pp.sBS*pp.Nresample));
                tmp = reshape(tmp,[pp.sBS*pp.Nresample nblocks Z]);
        ...
        SaveData(tl{n},[out_dir filesep fname '_EXPproc.mat']);
        tl{n} = [];
    end
%     matlabpool('close');
end
...
function [output a_indices] = iSubBlocks(signal,trigger,pp)
    ...
    t = cell(1,pp.NB);
    for n = 1:pp.NB
        a_indices{n} = roundn(IdentifyActuation(trigger(:,n)),-log10(pp.Nresample)); %actuation indices
        t{n} = diff(a_indices{n});
    end
    T = min(vertcat(t{:})); %period of actuation, in indices
    ...
    [~,M,~] = size(signal);
    nL = round(T*pp.Nresample);
    output = zeros(nL,M,NSB); %the rounding is really unnecessary, it just keeps Matlab from getting confused
    counter = 1;    
    for n = 1:pp.NB
        for nn = 1:length(a_indices{n})
            for q = 1:M
                sig = signal(floor(a_indices{n}(nn)):ceil(a_indices{n}(nn)+T),q,n);
                resampled = interp(sig,pp.Nresample);
                s_ind = round(mod(a_indices{n}(nn),1/pp.Nresample*10)*pp.Nresample)+1;
                output(:,q,counter) = resampled(s_ind:s_ind+nL-1);
            end
            counter = counter + 1;
        end
    end
...
function [sm_wave] = wavefilter(avg_wvfm,phys,pp)
    ...
    dt = 1/(pp.FS*pp.Nresample); %sampling time
    s0 = 1/(pp.FS); %smallest scale
    J1 = 5e2; %number of scales (minus 1)
    DJ = log2(3*N/2)/J1; %scale spacing, J1 = (LOG2(N DT/S0))/DJ.
    
    %smoothing params
    wt = 20*pp.Nresample; %search window temporal width 
    hww = 17*pp.Nresample;%15; %hard wavelet smoothing window
    sww = 9*pp.Nresample;%7; %soft wavelet smoothing window
    htw = 11*pp.Nresample;%15; %hard temporal smoothing window
    stw = 3*pp.Nresample;%7; %soft temporal smoothing window
    hwf = @hamming;
    swf = @hann;
    htf = @hamming;
    stf = @hann;
    ...
    center = ceil(N/2);
    i_a = rem(phys.i_a*pp.Nresample,N); %remove periods when acoustic indice > window length 
    shift = center - i_a;
