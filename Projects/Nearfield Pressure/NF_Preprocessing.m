...
%     pp.R = [2.45 2.57 2.69 2.90 3.19 3.46 3.68 3.28 3.15 2.63 2.56]; %radial distance of farfield microphones, in meters
    ...
    parfor n = 1:length(flist)
        ...
        if tl{n}.phys.Stdf ~= 0  %forced cases
            ...
            smp = Convert_AtP(tl{n}.trigger.a_i,[ffp nfp],sm_pressure,pp);
...
function [output] = Convert_AtP(a_i,p_p,sm_p_a,pp)
    ...
    nNBps = floor((NPa-1)*length(a_i{1})/NPp)*NB; %number of smoothed processing sub-blocks - decrement NPa by 1 to account for possible index-doublecounting
...
function SaveData(data,fname)
    ...
    save(fname,'nf','ff','phys','src_dir','filename','trigger');
end
