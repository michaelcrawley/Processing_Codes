function Wavelet_Decompose2DInst_V3(src_dir,flist,out_dir,a,windowfun,flag,cpus)
    tic;
    %Constants
    ...
    for k = 1:length(flist)
        ...
        parfor n = 1:N(3)            
            ...
            tmp = zeros(N(1),Nx);
            for q = 1:N(1)
                tmp(q,:) = interp1(x,sig(q,:,n),int_x,'pchip');
            end
%             tmp = interp2(X,T,sig(:,:,n),new_X,new_T,'spline');
        ...
        %Save Output
        save([out_dir,filesep,fname,' 2DWaveDecomp.mat'],'intwaveform','subsonic','supersonic','a','windowfun','int_x','data','ff','sub_filt','sup_filt');
    end
