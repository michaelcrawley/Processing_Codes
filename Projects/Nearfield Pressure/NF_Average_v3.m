function NF_Average_v3(src_dir,flist,out_dir,savename,cal_dir,params)
...
%actuator self noise. Outputs are saved in struct labeled 'pf'. This
%updated version will account for all (obtainable) forcing modes.
%Code version: 3.0
    ...
    for n = 1:N  
        ...
        %Get pulse locations for different modes
        pulse_phase_locs = CalcPulseLocations(pf.phys{n}.Mode1,pf.phys{n}.Mode2);
        
        %Read in data 
        [nfp,ffp,trigger] = ReadData(src_dir,flist{n},pf.pp,pf.phys{n},cal_dir); 
        
        %Average Nearfield waveform
        pf.avg_wvfm{n} = AvgWave([ffp nfp],trigger,pf.pp,pf.phys{n},pulse_phase_locs); %need to fix this line so that the pressure channels do not get reordered if nfp and ffp get reordered
        
        %Filter and smooth averaged nearfield waveforms
        pf.sm_wvfm{n} = NF_wavefilter(pf.avg_wvfm{n},pf.phys{n},pf.pp,pulse_phase_locs);
...
function [avg_waveform] = AvgWave(pressure,trigger,pp,phys,pulse_phase_locs)
        ...
        %Fix for trigger signal, due to EMI causing additional pulse
        triggerindex = pp.FS/pp.TF;
        ideallength = ceil(pp.FS/(phys.Stdf*phys.Ue/pp.D));
        EMIwidth = ideallength/length(pulse_phase_locs);
        Numpulse = find((1:length(pulse_phase_locs))*EMIwidth > triggerindex,1,'first');
        wndo = floor(T*length(pulse_phase_locs)/Numpulse); %length of averaging window, in indices, add in fix to make sure we're pulling the full waveform 

        %Identify actuation windows, sum waveform
        waveform = zeros(wndo,pp.NCh-1,pp.NB);
        for n = 1:pp.NB
            N = sum(a_indices{n}(:)+wndo < pp.BS); %number of complete waveforms for averaging
            for nn = 1:N
...
%to the 'phys' structure.
    ...
    %pull necessary data
    phys.Mode1 = str2double(md.m.value(2:3));
    phys.Mode2 = str2double(md.m.value(6:7));
...
function [pulselocs] = CalcPulseLocations(M1,M2,N)
    ...
    pulselocs = unique(pulselocs);
end

function [sm_wave] = NF_wavefilter(avg_wvfm,phys,pp,pulse_phase_locs)
    ...
    %Get pulse locations for different modes
    pulse_locs = round((pulse_phase_locs/2/pi)*N);
...
%     shift = center - i_a;
    ...
    for n = 1:M
        ...
        %normalize signal
        smean = mean(s); %mean for channel
        sn = s-smean; %normalized signal
        
        for q = 1:length(pulse_locs) %cycle through the different pulses for non-axisymmetric modes
                       
            %rotate signal so expected time of arrive is in center of window
            shift = center-(i_a+pulse_locs(q));
            sn = circshift(sn,shift(n));
            ...
            if ~isempty(ps)
                ...
                wave(ias(1):ias(2),iat(1):iat(2)) = hws(1:ias(2),hww+1:end-hww); %replace window
                if q == length(pulse_locs) %Perform soft-wavelet smoothing if on final pass-through
                    wave = ndnanfilter(wave,swf,[sww sww],[],{},{'circular'}); %soft smoothing in wavelet domain 
                end
            end
            ...
            filtered = sn;
            if ~isempty(ps)            
                rec = invcwt(wave,mother, scale,param,k)+smean; %reconstruct waveform from filtered wavelet coefs, add in signal mean
            ...
            %un-shift window
            sn = circshift(filtered,-shift(n));
        end
        sm_wave(:,n) = wmavg(sn,stw,stf,1);
    end  
end