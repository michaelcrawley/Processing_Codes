...
%Currently this function only works for instantaneous k-planes
...
%     PhavgcartDilatation = zeros([N(1:3),Nfp]);
    ...
    %partial u / partial x
    for n = 1:N(4)
        [~, px] = gradient(u.data(:,:,1,n),grid.y(1,:), grid.x(:,1)); %we have to switch the order, since the first output is du/dx in the HORIZONTAL direction
...
%     end    
    ...
    for n = 1:N(4)
        [py,~] = gradient(v.data(:,:,1,n),grid.y(1,:), grid.x(:,1));
...
%     end 
    ...
    save([src,filesep,'cartDilatation2C.mat'],'cartDilatation','grid');
...
%     save([src,filesep,'cartDilatation3C.mat'],'cartDilatation','grid','t','trigger');
...
%     end 
    ...
    for n = 1:N(4)
        [pr,~] = gradient(u_r.data(:,:,1,n),grid.r(1,:),grid.x(:,1));
        cylDilatation(:,:,1,n) = cylDilatation(:,:,1,n) + pr + u_r.data(:,:,1,n)./grid.r; 
    end
...
%     end
    ...
    %save out two-component dilatation...
    save([src,filesep,'cylDilatation2C.mat'],'cylDilatation','grid');
...
%     clear u_theta;
    ...
    %save out full dilatation field
%     save([src,filesep,'cylDilatation3C.mat'],'cylDilatation','grid','t','trigger'); 
end