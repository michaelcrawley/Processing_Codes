function FilterSignal(src_dir,flist,cal_dir,out_dir)
    ...
    for n = 1:length(flist)
        ...
        %Read in data 
        [nfp,ffp,trigger] = ReadData(src_dir,flist{n},pp,phys,cal_dir); 
        
        %Reshape data into forcing sub-blocks
        pressure = iSubBlocks([ffp(:,2,:) nfp],trigger,pp);
        ...
        for nn = 1:NSB
            fprintf('\t %d of %d \n',[nn NSB]);
            sm_pressure(:,:,nn) = wavefilter(pressure(:,:,nn),phys,pp);
        end
        
        save([out_dir filesep fname '_filt.mat'],'src_dir','filename','phys','pressure','sm_pressure');
    end
...
function [output] = iSubBlocks(signal,trigger,pp)
